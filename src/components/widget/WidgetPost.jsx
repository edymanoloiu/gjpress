			...
			<Tab.Container id="widget-post" defaultActiveKey="recent">
				<Nav variant="pills" className="row no-gutters">
					<Nav.Item className="col">
						<Nav.Link eventKey="recent">RECENTE</Nav.Link>
					</Nav.Item>
					<Nav.Item className="col">
						<Nav.Link eventKey="popular">POPULARE</Nav.Link>
					</Nav.Item>
				</Nav>

				<Tab.Content>
					<Tab.Pane eventKey="recent">
						{dataPost.slice(0, 8).map((data) => (
							<PostVideoTwo data={data} pClass="" key={data.slug} />
						))}
					</Tab.Pane>
					<Tab.Pane eventKey="popular">
						{dataPost.slice(0, 8).filter((data) => data.trending).map((data) => (
							<PostVideoTwo data={data} pClass="" key={data.slug} />
						))}
					</Tab.Pane>
				</Tab.Content>
